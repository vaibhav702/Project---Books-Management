...
const updateReviews = async function(req, res){
    try {
        ...
        if (!validator.isValidObjectId(reviewId)) {
            ...
            let reviewExists = await reviewModel.findOne({ reviewId: reviewId, isDeleted: false })
        ...
        const booksTotalReviews = await reviewModel.find({ bookId: bookId, isDeleted: false })
         ...
         const bookObject={
                    bookReviews,
                    reviewsData:booksTotalReviews

                }

        return res.status(200).send({status:true, message:"Done", data:bookObject})
